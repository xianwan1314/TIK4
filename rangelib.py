...
class RangeSet(object):
  ...
  lots of runs.)"""
    ...
    """
        out = []
        z = 0
        for p, d in merge(zip(self.data, itertools.cycle((+1, -1))),
    ...
    """
        ...
        z = 0
        for _, d in merge(zip(self.data, itertools.cycle((+1, -1))),
                          zip(other.data, itertools.cycle((+1, -1)))):
            if (z == 1 and d == 1) or (z == 2 and d == -1):
                return True
            z += d
    ...
    """
        ...
        offset = 0
        start = None
        for p, d in merge(zip(self.data, itertools.cycle((-5, +5))),
                          zip(other.data, itertools.cycle((-1, +1)))):
            if d == -5:
                start = p
            elif d == +5:
